...
public class MainView extends XdevView implements BroadcastListener, com.vaadin.server.ClientConnector.DetachListener {
	...
	BeanItemContainer<Fahrerfunktion> funktionen;
	/**
	 * 
	 */
	public MainView() {
	...
	private void addMultiSelectCombobox(){
		...
		funktionen = new BeanItemContainer<Fahrerfunktion>(Fahrerfunktion.class);
		funktionen.addAll(fahrerFunktion.findAll());
        ...
        comboBoxMultiselect.setContainerDataSource(funktionen);
		...
		setSelected(null);
		applyDriverFilter();
	    Component filter = horizontalLayout8.getComponent(0);
		
		
		Button but = new Button("Filter anwenden");
		...
		horizontalLayout8.addComponent(comboBoxMultiselect);
		horizontalLayout8.addComponent(but);
		horizontalLayout8.addComponent(filter);
	}
	
	private void applyDriverFilter(){
		tblEmployee.removeAllItems();
		Set<Fahrerfunktion> funktionen = (Set<Fahrerfunktion>) comboBoxMultiselect.getValue();
		tblEmployee.getBeanContainerDataSource().addAll(fahrerDAO.findAllByFunctionSet(funktionen));
		tblEmployee.sort();
	}
	
	private void setSelected(List<Fahrerfunktion> fahrerFunktionen){
		if(fahrerFunktionen != null){
			comboBoxMultiselect.unselectAll();
			comboBoxMultiselect.select(fahrerFunktionen);
		}else{
			comboBoxMultiselect.unselectAll();
			List<Fahrerfunktion> fahrerFunktionenBean = funktionen.getItemIds();
			fahrerFunktionenBean.forEach(s -> {
				if(s.getBeschreibung().matches("Stückgut|Umzug")){
					comboBoxMultiselect.select(s);
				}
			});
		}
	}
...
private void initUI() {
	...
	this.containerFilterComponent3.setFilterEnabled(false);
	...
	this.containerFilterComponent2.setWidth(400, Unit.PIXELS);
	this.containerFilterComponent2.setHeight(40, Unit.PIXELS);
	this.horizontalLayout8.addComponent(this.containerFilterComponent2);
	this.horizontalLayout8.setComponentAlignment(this.containerFilterComponent2, Alignment.MIDDLE_LEFT);
	this.horizontalLayout8.setExpandRatio(this.containerFilterComponent2, 100.0F);
	this.gridLayout4.setColumns(1);
	this.gridLayout4.setRows(2);
	this.horizontalLayout8.setWidth(-1, Unit.PIXELS);
	this.horizontalLayout8.setHeight(40, Unit.PIXELS);
	this.gridLayout4.addComponent(this.horizontalLayout8, 0, 0);
	this.gridLayout4.setComponentAlignment(this.horizontalLayout8, Alignment.MIDDLE_LEFT);
	...
	this.containerFilterComponent.setHeight(40, Unit.PIXELS);
...
private XdevHorizontalLayout horizontalLayout, horizontalLayout6, horizontalLayout5, horizontalLayout7,
		horizontalLayout2, horizontalLayout8, horizontalLayout3;
